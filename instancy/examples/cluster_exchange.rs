//! # Cluster Exchange Example
//!
//! Demonstrates `spawn_cluster()` with a two-node topology and an
//! `exchange_by_hash` edge. The hash function routes even values to node-a and
//! odd values to node-b, so records move across the simulated network before
//! being squared.
//!
//! Run with: `cargo run --all-features --example cluster_exchange`
...
async fn main() -> Result<()> {
    ...
    let input_a = vec![1, 2, 5, 6];
    let input_b = vec![3, 4, 7, 8];
    println!("node-a input: {input_a:?}");
    println!("node-b input: {input_b:?}");
    println!("routing: even values -> node-a, odd values -> node-b\n");
    ...
    assert_eq!(results_a, vec![4, 16, 36, 64]);
    assert_eq!(results_b, vec![1, 9, 25, 49]);

    println!("node-a output (even values squared): {results_a:?}");
    println!("node-b output (odd values squared):  {results_b:?}");
    println!(
        "\nThe outputs include values originally sent to both nodes, showing cross-node repartitioning."
    );
    println!("✓ Cluster completed successfully.");
