...
impl std::fmt::Debug for RuntimeConfig {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let mut s = f.debug_struct("RuntimeConfig");
        s.field("worker_threads", &self.worker_threads)
...
struct ClusterCancelHandle {
    /// Shared dataflow-level token. Cancelling this propagates to all child
    /// worker tokens AND triggers the distributed cancel broadcaster.
...
impl ClusterCancelHandle {
    fn cancel_with_reason(&self, reason: CancellationReason) {
        // Cancel the shared dataflow token first — this triggers the
        // distributed cancel broadcaster (Phase 8.5) which notifies
        // healthy peer nodes before we tear down bridges.
        self.dataflow_cancel.cancel_with_reason(reason.clone());
    ...
    fn is_cancelled(&self) -> bool {
        self.bridge_cancel.is_cancelled()
            || self.worker_tokens.first().is_some_and(|t| t.is_cancelled())
    }
}
    ...
    fn register(
        ...
        for peer_id in peer_node_ids {
            ...
            bucket.push(PeerRegistration {
                ...
                cancel_handle: ClusterCancelHandle {
                    dataflow_cancel: cancel_handle.dataflow_cancel.clone(),
...
pub struct RuntimeHandle {
    ...
    /// Wrapped in Mutex so `start_membership_listener()` can replace it atomically.
...
impl RuntimeHandle {
    ...
    pub fn new(config: RuntimeConfig) -> Result<Self> {
        ...
        // Extract topology and membership from config.
        #[cfg(feature = "transport")]
        let (initial_topology, membership_provider) = {
            match config.topology {
                Some(mut topo) => {
                    let provider = topo.take_membership();
                    (Some(topo), provider)
                }
                None => (None, None),
            }
        ...
        #[cfg(feature = "transport")]
        if let Some(provider) = membership_provider {
            handle.start_membership_listener(provider)?;
        }
    ...
    ///
    /// Returns `None` if no topology was provided via [`RuntimeConfig`].
    ///
    ...
    /// Start a background membership listener that processes node join/leave events.
    ///
    /// Called automatically during construction if the topology has a membership
    /// provider attached.
    #[cfg(feature = "transport")]
    fn start_membership_listener(
        &self,
        membership: Box<dyn crate::execute::ClusterMembership>,
    ) -> Result<()> {
        use crate::execute::MembershipEvent;

        let mut rx = membership.events().ok_or_else(|| {
            Error::Runtime(RuntimeError::AlreadyConsumed {
                resource: "membership provider events".into(),
            })
        })?;
        ...
        F: Fn(&mut DataflowBuilder<T>) -> Result<()>,
    {
        if options.auto_parallelism {
            ...
            build(&mut probe_builder)?;
            let probe_df = probe_builder.build()?;
            validate_uniform_stage_parallelism(&probe_df, num_workers)?;
            drop(probe_df);
    ...
    fn prepare_worker<T: Timestamp>(
        ...
        {
            if ghost_operators.contains(&info.operator_index) {
                continue;
            }
    ...
    fn spawn_multi_internal<T, F>(
        ...
        stage_parallelism: Option<Vec<usize>>,
    ) -> Result<MultiSpawnedDataflow<T>>
    where
        T: Timestamp,
        F: Fn(&mut DataflowBuilder<T>) -> Result<()>,
    {
        ...
        let stage_parallelism = match stage_parallelism {
            Some(stage_parallelism) => {
                if stage_parallelism.len() != dataflow.stages().len() {
                    return Err(Error::Runtime(RuntimeError::InvalidConfig(format!(
                        "stage parallelism metadata length {} does not match built dataflow stage count {}",
                        stage_parallelism.len(),
                        dataflow.stages().len()
                    ))));
                }
                stage_parallelism
            }
            None => resolve_stage_parallelism(dataflow.stages(), default_parallelism),
        };
        ...
        if num_workers > 1 {
            ...
            let regular_edges = dataflow.graph.edges();
            for (edge_idx, edge_capacity, mut creator) in creators {
                ...
                let (source_par, target_par) = if edge_idx < regular_edges.len() {
                    let edge = &regular_edges[edge_idx];
                    let source_par = stage_parallelism
                        .get(edge.source_stage.index())
                        .copied()
                        .unwrap_or(default_parallelism);
                    let target_par = stage_parallelism
                        .get(edge.target_stage.index())
                        .copied()
                        .unwrap_or(default_parallelism);
                ...
                if shared_factories.len() != expected_count {
                    return Err(Error::Runtime(RuntimeError::InvalidConfig(format!(
                        "exchange factory creator for edge {edge_idx} produced {} factories, expected {} (max of source_par={}, target_par={})",
        ...
        let mut control_pairs: Vec<Option<(ControlSender, ControlReceiver)>> = if num_workers > 1 {
            let df_cancel = dataflow_cancel
                .as_ref()
                .expect("dataflow_cancel set for multi-worker")
                .clone();
        ...
        for worker_idx in 0..num_workers {
            ...
            let ghost_operators: std::collections::HashSet<usize> = dataflow
                .stages()
                .iter()
                .filter(|stage| {
                    let parallelism = stage_parallelism
                        ...
                        .copied()
                        .unwrap_or(default_parallelism);
                    worker_idx >= parallelism
                })
                .flat_map(|stage| stage.operator_indices.iter().copied())
                .collect();
        ...
        // Phase 6: Register all workers NOW.
        ...
        F: Fn(&mut DataflowBuilder<T>) -> Result<()>,
    {
        ...
        build(&mut probe_builder)?;
        let probe_df = probe_builder.build()?;
        let stage_parallelism = resolve_stage_parallelism(probe_df.stages(), default_parallelism);
        drop(probe_df);
        ...
        W: tokio::io::AsyncWrite + Unpin + Send + 'static,
    {
        ...
        validate_uniform_stage_parallelism(&dataflows[0], total_workers)?;

        // Phase 3: Compute fingerprint and create transport.
        ...
        // Create transport and extract receivers based on transport mode.
        // After this block: `transport`, `receivers`, `control_receivers` are set.
        // For dedicated mode, `dedicated_session` holds the raw session for handshake/barrier.
        let transport: Arc<ClusterTransport>;
        ...
        let dedicated_session: Option<Arc<TransportSession>>;
        // Error receivers from shared mode (for monitoring tasks).
        let shared_error_receivers: Option<
        ...
        let shared_managers_arc: Option<
            Arc<
                std::collections::HashMap<
                    String,
        ...
        match transport_config {
            ClusterSpawnTransport::Dedicated {
                ...
                // Validate connections match topology.
                {
                ...
                // Extract control receivers.
                for (peer_id, peer_map) in raw_receivers.iter_mut() {
            ...
            ClusterSpawnTransport::Shared {
                ...
                // Compute channel IDs to register.
        ...
        if let Some(error_rxs) = shared_error_receivers {
            // SAFETY: dataflow_cancel is unconditionally set for cluster dataflows
            let df_cancel = dataflow_cancel
                .as_ref()
        ...
        let peer_reg_id = if !remote_peer_ids.is_empty() {
            ...
            let cancel_handle = ClusterCancelHandle {
                dataflow_cancel: dataflow_cancel
                    .as_ref()
                    .expect("dataflow_cancel always set for cluster dataflows")
                    .clone(),
                worker_tokens,
...
impl SimpleRuntime {
        ...
        F: Fn(&mut DataflowBuilder<T>) -> Result<()>,
    {
        ...
        // Phase 2b: Legacy dedicated-thread spawn requires uniform stage parallelism.
        validate_uniform_stage_parallelism(&dataflows[0], num_workers)?;
...
impl DataflowCompletion {
    ...
    pub fn wait(self) -> Result<()> {
        let mut state = self.shared.lock().map_err(|_| Error::LockPoisoned {
            context: "completion mutex poisoned".into(),
        })?;
        while state.result.is_none() {
            state = self.condvar.wait(state).map_err(|_| Error::LockPoisoned {
                context: "completion mutex poisoned during wait".into(),
            })?;
        }
        interpret_completion(state.result.take().ok_or_else(|| {
            Error::Runtime(RuntimeError::InvalidConfig(
                "internal: result missing after wait loop".into(),
            ))
        })?)
    }
}
...
impl Future for DataflowCompletion {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        let mut state = match self.shared.lock() {
            Ok(s) => s,
            Err(_) => {
                return Poll::Ready(Err(Error::LockPoisoned {
                    context: "completion mutex poisoned".into(),
                }));
            }
        };
...
impl<T: Timestamp> MultiSpawnedDataflow<T> {
    ...
    ///     b.input::<i32>("data").unwrap().output("out").unwrap(); Ok(())
    /// }).unwrap();
    ...
    pub fn take_all_inputs<D: Clone + Send + 'static>(
        ...
        for w in &mut self.workers {
            senders.push(
                // SAFETY: port names were validated to exist in the preceding loop
                w.take_input::<D>(name)
    ...
    fn validate_port<C: 'static>(
        ...
        match ports.iter().find(|(n, _, _)| n == name) {
            ...
            Some((_, port_type, _)) if *port_type != type_name => {
                Err(Error::Runtime(RuntimeError::InvalidConfig(format!(
                    "worker {worker_idx} {direction} port '{name}' has type {port_type}, but requested {type_name}"
                ))))
            }
            Some((_, _, any_box)) if !any_box.is::<C>() => {
                Err(Error::Runtime(RuntimeError::InvalidConfig(format!(
...
impl<T: Timestamp> ClusterSpawnedDataflow<T> {
    ...
    pub fn take_input<D: Clone + Send + 'static>(
        ...
        self.inner
            ...
            .ok_or_else(|| {
                Error::Runtime(RuntimeError::AlreadyConsumed {
                    resource: "dataflow join handle".into(),
                })
            })?
    ...
    pub fn join(mut self) -> Result<ClusterCompletion<T>> {
        let completion = self
            .inner
            .take()
            .ok_or_else(|| {
...
impl<T: Timestamp> ClusterCompletion<T> {
    ...
    pub fn wait(mut self) -> Result<()> {
        let completion = self.completion.take().ok_or_else(|| {
            Error::Runtime(RuntimeError::AlreadyConsumed {
                resource: "cluster completion".into(),
            })
        })?;
...
impl<T: Timestamp> Future for ClusterCompletion<T> {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        let Some(completion) = this.completion.as_mut() else {
            return Poll::Ready(Err(Error::Runtime(RuntimeError::AlreadyConsumed {
                resource: "cluster completion".into(),
            })));
        };
...
/// Resolve each inferred stage's effective parallelism.
fn resolve_stage_parallelism(
    stages: &[crate::dataflow::stage::StageInfo],
    default_parallelism: usize,
) -> Vec<usize> {
    stages
        .iter()
        .map(|stage| stage.parallelism.unwrap_or(default_parallelism))
        .collect()
}

/// Validate that every explicit stage parallelism matches the uniform worker count.
fn validate_uniform_stage_parallelism<T: Timestamp>(
    ...
    for stage in dataflow.stages() {
        if let Some(parallelism) = stage.parallelism {
            if parallelism != num_workers {
                return Err(Error::Runtime(RuntimeError::InvalidConfig(format!(
                    "stage {} has explicit parallelism {} but the runtime is spawning {} workers",
...
fn materialize_executor<T: Timestamp>(
    ...
    let mut worker_subgraph_builder = dataflow.subgraph_builder.clone();
    if !ghost_operators.is_empty() {
        worker_subgraph_builder.mark_ghost_operators(ghost_operators);
    }
...
mod tests {
    ...
    fn simple_runtime_rejects_input_ports_on_run() {
        ...
        let _ = builder.input::<i32>("data").unwrap().output("out").unwrap();
        let dataflow = builder.build().unwrap();
    ...
    fn runtime_handle_spawn_accepts_input_ports() {
        ...
        let input = builder.input::<i32>("x").unwrap();
        input.map("inc", |_t, x| x + 1).output("y").unwrap();
        let dataflow = builder.build().unwrap();
    ...
    async fn spawn_async_roundtrip() {
        ...
        let input = builder.input::<i32>("data").unwrap();
        input.map("mul10", |_t, x| x * 10).output("out").unwrap();
        let dataflow = builder.build().unwrap();
    ...
    fn spawn_multi_single_worker_matches_spawn() {
        let rt = SimpleRuntime::new();
        let mut multi = rt
            .spawn_multi("test", 1, |builder: &mut DataflowBuilder<u64>| {
                let input = builder.input::<i32>("data").unwrap();
    ...
    fn spawn_multi_parallel_workers() {
        ...
        let num = 4;
        let mut multi = rt
            .spawn_multi("parallel", num, |builder: &mut DataflowBuilder<u64>| {
                let input = builder.input::<i32>("data").unwrap();
                input.map("triple", |_t, x| x * 3).output("out").unwrap();
                Ok(())
            })
            .unwrap();
    ...
    fn spawn_multi_cancel_stops_all() {
        let rt = SimpleRuntime::new();
        let mut multi = rt
            .spawn_multi("cancel-test", 3, |builder: &mut DataflowBuilder<u64>| {
                let input = builder.input::<i32>("data").unwrap();
                input.output("out").unwrap();
                Ok(())
            })
    ...
    fn spawn_multi_join_returns_completion() {
        let rt = SimpleRuntime::new();
        let multi = rt
            .spawn_multi("join-test", 2, |builder: &mut DataflowBuilder<u64>| {
    ...
    fn context_survives_build() {
        ...
        let input = builder.input::<i32>("data").unwrap();
        let _out = input.output("sink").unwrap();
        let dataflow = builder.build().unwrap();
    ...
    fn control_broadcast_worker_error_cancels_siblings() {
        ...
        let mut multi = rt
            .spawn_multi("ctrl-err", 2, move |builder: &mut DataflowBuilder<u64>| {
                let input = builder.input::<i32>("data").unwrap();
    ...
    fn control_broadcast_single_worker_no_overhead() {
        ...
        let rt = SimpleRuntime::new();
        let mut multi = rt
            .spawn_multi("single", 1, |builder: &mut DataflowBuilder<u64>| {
                let input = builder.input::<i32>("data").unwrap();
                input.map("inc", |_t, x: i32| x + 1).output("out").unwrap();
                Ok(())
            })
            .unwrap();
    ...
    fn control_broadcast_unit_test_sender_receiver() {
        ...
        let (senders, mut receivers) = ControlBroadcast::new(3, &wakes, df_cancel.clone()).unwrap();
    ...
    fn control_broadcast_limit_does_not_cancel() {
        ...
        let (senders, _receivers) = ControlBroadcast::new(2, &wakes, df_cancel.clone()).unwrap();
    ...
    fn source_async_basic_roundtrip() {
        ...
        pipe.map("double", |_t, x| x * 2).output("out").unwrap();
        let dataflow = builder.build().unwrap();
    ...
    fn source_async_with_runtime_handle() {
        ...
        pipe.map("inc", |_t, x| x + 100).output("out").unwrap();
        let dataflow = builder.build().unwrap();
    ...
    async fn test_unary_async_error_propagation() {
        ...
        let logic = Arc::new(|_time: u64, _batch: Vec<i32>| async move {
            Err(Error::Runtime(RuntimeError::InvalidConfig(
                "async failure".into(),
            )))
        });
    ...
    mod membership_tests {
        ...
        fn test_runtime_with_topology(topology: ClusterTopology) -> RuntimeHandle {
            RuntimeHandle::new(RuntimeConfig {
                worker_threads: 1,
                name: "membership-test".to_string(),
        ...
        fn test_runtime_with_membership(
            topology: ClusterTopology,
        ) -> (
            RuntimeHandle,
            tokio::sync::mpsc::UnboundedSender<MembershipEvent>,
        ) {
            ...
            let rt = RuntimeHandle::new(RuntimeConfig {
                worker_threads: 1,
                name: "membership-test".to_string(),
                topology: Some(topology.with_membership(membership)),
                ..Default::default()
            })
            .unwrap();
            (rt, tx)
        ...
        fn set_and_get_topology() {
            ...
            .unwrap();
            let rt = test_runtime_with_topology(topo.clone());
        ...
        async fn membership_node_join_updates_topology() {
            let (rt, tx) = test_runtime_with_membership(ClusterTopology::single_node(2));
        ...
        async fn membership_node_leave_updates_topology() {
            let (rt, tx) = test_runtime_with_membership(
        ...
        async fn membership_node_leave_then_rejoin() {
            ...
            let topo = rt.current_topology().unwrap();
            assert!(topo.contains_node("node-b"));
            assert_eq!(topo.total_workers(), 7);
        }

        #[tokio::test]
        async fn membership_rapid_events_processed_in_order() {
            let (rt, tx) = test_runtime_with_membership(ClusterTopology::single_node(1));
        ...
        async fn membership_runtime_shutdown_stops_listener() {
            let (rt, _tx) = test_runtime_with_membership(ClusterTopology::single_node(1));
        ...
        fn set_membership_rejects_consumed_provider() {
            ...
            let topology = ClusterTopology::single_node(1).with_membership(membership);

            let result = RuntimeHandle::new(RuntimeConfig {
                worker_threads: 1,
                name: "membership-test".to_string(),
                topology: Some(topology),
                ..Default::default()
        ...
        #[tokio::test]
        async fn node_joined_zero_workers_ignored() {
            let (rt, tx) = test_runtime_with_membership(ClusterTopology::single_node(1));
            ...
            let topo = rt.current_topology().unwrap();
            assert_eq!(topo.node_count(), 1);
            assert_eq!(topo.total_workers(), 1);
            assert!(!topo.contains_node("bad-node"));
        }

        #[tokio::test]
        async fn node_left_removes_last_node_clears_topology() {
            let (rt, tx) = test_runtime_with_membership(
                ClusterTopology::multi_node(vec![NodeConfig::new("only-node", 2)]).unwrap(),
            );
